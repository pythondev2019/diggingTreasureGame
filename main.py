...
def tick_routine(redraw=False):
    def get_player_img():
        return player[
                      ("Left" if game.player.left else "Right")
                      + ("Hurt" if game.player.life_restore else "Normal")
                      ]
    ...
    for y in range(logics.GY):
        for x in range(logics.GX):
            if (
                redraw
                or game.g[y][x] is not onscreen[y][x]
                or game.g[y][x] == logics.Elem.player
                ):
                onscreen[y][x] = game.g[y][x]
                if dg[y][x]:
                    canvas.delete(dg[y][x])
                if game.g[y][x] == logics.Elem.player:
                    dg[y][x] = canvas.create_image(
                                                   x * cellSize, y * cellSize, anchor="nw", image=get_player_img()
                                                   )
                else:
                    dg[y][x] = canvas.create_image(
                                                   x * cellSize,
...
canvas.yview_moveto(
                    (borderSize + game.player.y - 3.5) / (logics.GY + 2 * borderSize)
                    )
    canvas.xview_moveto(
                        (borderSize + game.player.x - 4.5) / (logics.GX + 2 * borderSize)
                        )
...
hpbar = Progressbar(
                    f, orient=HORIZONTAL, length=100, value=100, maximum=100, mode="determinate"
                    )
hpbar.grid(row=0, column=0)
Label(f, text="Life").grid(row=0, column=1)
Label(f).grid(row=0, column=2, sticky="we")
moneybar = Progressbar(
                       f, orient=HORIZONTAL, length=80, value=0, maximum=1, mode="determinate"
                       )
...
tk.bind_all("", lambda *_: cmd(logics.Command.left))
tk.bind_all("", lambda *_: cmd(logics.Command.right))
tk.bind_all("", lambda *_: cmd(logics.Command.next))
tk.bind_all("", lambda *_: cmd(logics.Command.down))
