...
class Fire:
    ...
    def tick(self):
        ...
        if self.y < GY - 1 and self.game.g[self.y + 1][self.x] == Elem.empty:  # go down
            ...
            self.game.g[self.y][self.x] = Elem.empty
            self.y += 1
        elif self.y < GY - 1 and self.game.g[self.y + 1][self.x] in Elem.chunk + [
                                                                                  Elem.heart
                                                                                  ]:  # burn down
            self.game.new_fire(self.y + 1, self.x)
                                                                                  elif (
                                                                                        self.y < GY - 1 and self.game.g[self.y + 1][self.x] == Elem.player
                                                                                        ):  # beat down
            if self.game.player.hurt(BURN_LIFE):
                raise _FireStopped()
                                                                                  elif (
                                                                                        0 < self.x < GX - 1
                                                                                        and self.game.g[self.y][self.x + 1] in _t
                                                                                        and self.game.g[self.y][self.x - 1] in _t
                                                                                        ):  # nothing
                return
    elif (self.x == 0 and self.game.g[self.y][1] in _t) or (
                                                            self.x == GX - 1 and self.game.g[self.y][GX - 2] in _t
                                                            ):  # nothing
        return
        else:
            if self.x == 0:
...
class Game:
    ...
    def init_level(self, level):
        ...
        self.g = [
                  [
                   Elem.dirt if random.random() > EVIL_PROB else Elem.evildirt
                   for y in range(GX)
                   ]
                  for x in range(GY)
                  ]
                  ...
                  # put goods
                  available = [
                               (y, x) for y in range(GY) for x in range(GX) if self.g[y][x] == Elem.dirt
                               ]
                  ...
                  self.level = level
                      self.cur = 0
                          ...
                          self.fires = set()
                          self.player = Player(
                                               self, 0, math.floor(GX / 2), self.player.name, self.player.total_score
                                               )
